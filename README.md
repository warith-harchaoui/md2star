...
- **Template-Intelligent PPTX**: `md2pptx --smart-layout --template <designer.pptx>` maps each slide onto a real designer template's own named layouts (a marketing deck routinely ships 20-40) instead of Pandoc's ~7 generic ones. An LLM picks the best-fitting layout per slide from a catalog of the template's own layouts; a **vision model visually confirms or overrides** that pick against the candidates' actual rendered thumbnails, not just their captions; a `python-pptx` assembler then builds the deck directly on the chosen layout. Add `--eyeball-iterations N` for a target-matching Ralph Eyeball Loop that compares each rendered slide against its chosen layout's own thumbnail and applies bounded fixes (shrink overflow text, cap bullets, grow an undersized image). Opt-in (`pip install 'md2star[pptx]'`) and best-effort, same engine contract as the LLM Linter below. See [EXAMPLES.md §12](https://github.com/warith-harchaoui/md2star/blob/main/EXAMPLES.md#12-template-intelligent-pptx).
...
A separate project, [`md2star-rs`](https://github.com/warith-harchaoui/md2star-rs), reaches for
the same "Markdown in, faithful DOCX/PPTX out" goal without Pandoc: a pure-Rust reader → AST →
writer pipeline, compiled into a single static binary with zero runtime dependency — no Pandoc
install, so it runs on phones, locked-down machines, and (eventually) WASM. The trade-off: it's
a focused subset, not a replacement — no bibliography/citations (no stable Rust CSL processor
yet), no math→OMML, no embedded images or hyperlink relations in the PPTX backend. Use
`md2star-rs` when you need a zero-install single binary for straightforward Markdown →
Word/PowerPoint; keep using `md2star` (this package) for citations, math, and maximum fidelity.
