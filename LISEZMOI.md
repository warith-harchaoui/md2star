...
- **PPTX intelligent gabarit** : `md2pptx --smart-layout --template <gabarit-designer.pptx>` mappe chaque diapositive sur les vraies mises en page nommées d'un gabarit de designer (une présentation marketing en propose typiquement 20 à 40) au lieu des ~7 mises en page génériques de Pandoc. Un LLM choisit la mise en page la plus adaptée pour chaque diapositive à partir d'un catalogue des mises en page du gabarit ; un modèle de vision **confirme ou corrige visuellement** ce choix en comparant les vraies vignettes rendues des candidats, pas seulement leurs légendes ; un assembleur `python-pptx` construit ensuite la présentation directement sur la mise en page choisie. Ajoutez `--eyeball-iterations N` pour une boucle de comparaison visuelle guidée par la cible qui compare chaque diapositive rendue à la vignette de sa mise en page choisie et applique des corrections bornées (réduire un débordement de texte, limiter les puces, agrandir une image trop petite). Opt-in (`pip install 'md2star[pptx]'`) et non bloquant, même contrat de moteur que le Linter LLM ci-dessous. Voir [EXAMPLES.md §12](https://github.com/warith-harchaoui/md2star/blob/main/EXAMPLES.md#12-template-intelligent-pptx).
